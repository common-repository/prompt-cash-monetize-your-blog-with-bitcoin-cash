...
class TemplateEngine {
	...
	public function showMetaboxPayment($post = null, $args = array()) {
		$btnCurrencyPlaceholder = __("USD", 'ekliptor');
		$defAmountPlaceholder = __("0.1", 'ekliptor');
		$tipTextPlaceholder = __("Message on the tip button...", 'ekliptor');
		$hideTipTextPlaceholder = __("Message before hidden text...", 'ekliptor');
    	include PRCA__PLUGIN_DIR . 'tpl/admin/metaboxPayment.php';
    }
    
    public function showMetaboxAccount($post = null, $args = array()) {
		$accountTokenPlaceholder = __("123-abcfkq", 'ekliptor');
    	include PRCA__PLUGIN_DIR . 'tpl/admin/metaboxAccount.php';
    }
    
    public function showMetaboxBlurryImage($post = null, $args = array()) {
    	$tipImagePlaceholder = __('the text shown to users on your website next to the image...', 'ekliptor');
    	$cacheMbPlaceholder = __('the max cache size in mb...', 'ekliptor');
    	include PRCA__PLUGIN_DIR . 'tpl/admin/metaboxBlur.php';
    }
    ...
    public function showMetaboxAdvanced($post = null, $args = array()) {
    	...
    	include PRCA__PLUGIN_DIR . 'tpl/admin/metaboxAdvanced.php';
    }
    ...
    public function getTemplate(string $templateFile, array $tplVars = array()): string {
    	ob_start();
    	include PRCA__PLUGIN_DIR . 'tpl/' . $templateFile;
		...
		if (isset($selectOptions['multicheck']) && $selectOptions['multicheck'] === true) {
			...
			$output = vsprintf('<div class="ct-input-wrapper">
						<div class="ct-input-label-side">%s</div>
                    		...
                    		$label,
                    		__('Check All', 'ekliptor'),
                    		__('Uncheck All', 'ekliptor'),
                    		$select_options,
                    ]);
			return $output;
		}
